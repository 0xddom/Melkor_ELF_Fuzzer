...
unsigned int orcn = 0; // OrcN inside the for() loop. fuzz_* modules will use it
                       // through different loops
unsigned int n = 5000; // Default for option -n
unsigned int quiet =
    0; // For quiet mode (-q). Default is not quiet [debug() output]
unsigned int likelihood =
    10; // Likelihood given in % of the execution of each rule in the main for()
        // loop in fuzz_*.c. Default 10%
unsigned int like_a = 10,
             like_b = 1; // Based upon the likelihood, these will be used for:
                         // rand() % like_a < like_b. Default values for 10%
unsigned int secnum =
    0; // Used in loops here but refered in fuzz_*.c as the section number
unsigned int entry = 0; // Used in loops here but refered in fuzz_*.c as the
                        // entry number inside a section (for DYN, SYM, REL)
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "aHSPsDRNZABqn:l:")) != EOF)
    ...
    case 'l':
      ...
      if (likelihood < 1 || likelihood > 100) {
        fprintf(stderr, "[!] Likelihood (-l) is given in %% and must be "
                        "between 1 and 100\n");
        exit(EXIT_FAILURE);
      }
  ...
  if ((elfptr = (char *)mmap(NULL, elfstatinfo.st_size, PROT_READ, MAP_SHARED,
                             elffd, 0)) == MAP_FAILED) {
    perror("mmap");
    close(elffd);
    exit(EXIT_FAILURE);
  ...
  if (mkdir(dirname, 0775) == -1)
    if (errno == EEXIST)
      printf(
          "[!] Dir '%s' already exists. Files inside will be overwritten !\n",
          dirname);
  ...
  for (orcn = 1; orcn <= n; orcn++) {
    ...
    snprintf(dirname_orcfname, sizeof(dirname) + sizeof(orcfname) + 2, "%s/%s",
             dirname, orcfname);
    ...
    if ((orcptr = (char *)mmap(NULL, elfstatinfo.st_size,
                               PROT_READ | PROT_WRITE, MAP_SHARED, orcfd, 0)) ==
        MAP_FAILED) {
      perror("mmap");
      close(orcfd);
      continue;
    ...
    if (mode & REL) {
      ...
      for (secnum = 0; secnum < orcHDR->e_shnum; secnum++, orcSHT++) {
        ...
        printf("\n[+] Fuzzing the relocations section %s with %d %s entries\n",
               orcptr + orcshstrtab_offset + orcSHT->sh_name,
               (unsigned int)(orcSHT->sh_size / orcSHT->sh_entsize),
    ...
    if (mode & SYM) {
      ...
      for (secnum = 0; secnum < orcHDR->e_shnum; secnum++, orcSHT++) {
        ...
        linkstrtab_section =
            *(Elf_Shdr *)(orcptr + orcHDR->e_shoff +
                          (orcSHT->sh_link * sizeof(Elf_Shdr)));
        ...
        for (entry = 0; entry < orcSHT->sh_size / orcSHT->sh_entsize;
             entry++, elfSYM++, orcSYM++) {
          fuzz_sym();
    ...
    if (mode & DYN) {
      ...
      for (secnum = 0; secnum < orcHDR->e_shnum; secnum++, orcSHT++) {
        ...
        for (entry = 0; entry < orcSHT->sh_size / orcSHT->sh_entsize;
          ...
          if (elfDYN->d_tag ==
              DT_NULL) // End of _DYNAMIC[]. Trust in elfDYN, orcDYN->d_tag =
                       // NULL might have been changed
            break;
        }
      }
...
void usage(const char *self) {
  ...
  printf("Usage: %s <ELF metadata to fuzz> <ELF file template> [-n num -l "
         "likelihood -q]\n",
         self);
  ...
  printf("\t\t-B  All of the above (except -a [Autodetect] and -H [ELF "
         "Header])\n");
  printf(
      "\t-n  Number of new fuzzed ELF files (orcs) to create (default: %d)\n",
      n);
  printf("\t-l  Likelihood (given in %% from 1-100) of the execution of each "
         "fuzzing rule (default: %d%%)\n",
         likelihood);
  printf("\t-q  Quiet mode (doesn't print to STDOUT every executed fuzzing "
         "rule)\n");
