...
// Array of function pointers. Index zero won't be used. The fuzzing rules start
// from index 1
func_ptr sht[N_RULES_SHT + 1];
int sh = 0; // Section Header. Used inside the for() loop in fuzz_sht() and will
...
int sht3(void) {
  ...
  case SHT_DYNSYM:
    if (mode &
        (SYM | REL)) // In REL, sh_offset of the symbol table will be needed
      return 0;
    break;
  case SHT_RELA:
  case SHT_REL:
...
int sht5(void) {
  ...
  fprintf(logfp, "(SHT[%d]->sh_addralign = 0x" HEX ")", sh,
          orcSHT->sh_addralign);
...
int sht7(void) {
  ...
  case SHT_DYNSYM:
    if (mode &
        (SYM | REL)) // In REL, sh_type of the symbol table will be needed
      return 0;
    break;
  case SHT_RELA:
  case SHT_REL:
...
int sht11(void) {
  if (orcSHT->sh_type != SHT_DYNAMIC && orcSHT->sh_type != SHT_HASH &&
  ...
  if (orcSHT->sh_type == SHT_DYNAMIC) {
    ...
    if (rand() % 4 < 3) {
      while ((l = 1 + (rand() % (orcHDR->e_shnum - 1))))
        if (orcOrigSHT[l].sh_type != SHT_SYMTAB &&
            orcOrigSHT[l].sh_type != SHT_DYNSYM)
          break;
    } else
      l = (Elf_Word)getElf_Half();
...
int sht16(void) {
  if (orcSHT->sh_type != SHT_HASH && orcSHT->sh_type != SHT_GNU_HASH)
    return 0;
...
int sht20(void) {
  ...
  if (r % 3 == 0) // If running in 64 bits, set a size modulo the 32-bits
                  // struct's size and viceversa
#if defined(__i386__)
    s = sizeof(Elf64_Nhdr);
#elif defined(__x86_64__) || defined(__aarch64__)
    s = sizeof(Elf32_Nhdr);
...
int sht25(void) {
  ...
  // Using the original sh_offset of the ELF. sh_offset in the ORC might be
  // already fuzzed
  strncpy(orcptr + elfSHT[interp].sh_offset, dirname_orcfname,
          strlen(dirname_orcfname));
...
int sht28(void) {
  ...
  if (rand() % 2) {
    ...
    if (mode &
        REL) // If REL is fuzzed, the sh_offset of the string tab will be needed
      if (rand() % 3 < 2)
        return 0;
...
int sht35(void) {
  ...
  if (orcSHT->sh_type != SHT_INIT_ARRAY && orcSHT->sh_type != SHT_FINI_ARRAY)
    return 0;
...
int sht37(void) {
  ...
  if (tbss && tdata) {
    fprintf(logfp, "(SHT[%d]->sh_flags = 0x" HEX ",", tdata,
            orcOrigSHT[tdata].sh_flags);
    fprintf(logfp, " SHT[%d]->sh_flags = 0x" HEX ")", tbss,
            orcOrigSHT[tbss].sh_flags);
  } else if (tbss)
    fprintf(logfp, "(SHT[%d]->sh_flags = 0x" HEX ")", tbss,
            orcOrigSHT[tbss].sh_flags);
  else
...
// Previous rules might already fuzzed sh_name and could SIGSEGV here is orc
// sh_name is used
...
void fuzzFlags() {
  ...
  if (r % 3 == 0) {
    ...
    if (r % 5 == 0)
      orcSHT->sh_flags =
          (SHF_WRITE | SHF_ALLOC | SHF_EXECINSTR | SHF_TLS | SHF_MASKPROC);
    else if (r % 5 == 1)
      orcSHT->sh_flags |= SHF_WRITE;
    else if (r % 5 == 2)
      orcSHT->sh_flags |= SHF_ALLOC;
